...
file_path = (
    f"./{folder_path}/{file_number}/error_address_{file_number}.csv"  # 입력된 파일 경로
)
output_folder = (
    f"./{folder_path}/{file_number}/post_error_{file_number}"  # 동일한 폴더에 결과 저장
)
...
def process_addresses(df):
    ...
    for idx, address in tqdm(
        df["주소"].items(), total=len(df), desc="Processing addresses"
    ):
...
def main():
    ...
    # 결과 저장: 동일한 폴더에 저장
    final_result_path = os.path.join(output_folder, f"post_final_{file_number}.csv")
    error_result_path = os.path.join(output_folder, f"post_error_{file_number}.csv")
